...
\newcommand{\relplonk}{\ensuremath{\rel_{app}}\xspace}
...
contract address with a merkle root of a tree whose leaves are the verification keys of the contract functions.
...
\end{itemize}
 From what is written so far, one could ask why not \emph{only} have a final predicate that includes the transition checks. 
The point is that in our usecase the final predicate is applied to small parts of each witness - namely the note opreations, and is relatively simple. Thus, this decomposition facilitates obtaining better prover efficiency, compared to what we'd get from a monolithic circuit for the whole computation.
...
$S=n+d+\ell$
Relation \relplonk of $\cm1,\ldots,\cm_S$
...
$f:D^*\times V \to \set{\acc,\rej}$ is the \emph{final predicate}.

The relation $\rel=\rel_{F,f}$ consists of pairs $(\inp,\wit)$ such that
...
\begin{itemize}
 ...
 \item $f(s_1,\ldots,s_n,\finstate)=\acc$.
\end{itemize}


we say a \zksnark for \rel is \emph{space-efficient} if given $s$ and streaming access
to $z$ and $w$ \prv requires space $O(|F|+|\finstate|)$.




\section{Record operations}
*Records* are pairs $(v,c)$ - $v$ is a value, $c$ is a counter.
A record operation has one of the following forms
$(\add,v,c)$,$(\del,v,vc,c)$,$(\read,v,vc,c)$.

Where $v$ denotes a note value, $c$ denotes the counter of the operation.
For $\read/\del$ - $vc$ denotes the counter where the value being read or deleted 



We say a sequence $O$ of record ops of size $n$ is *consistent* if 
- The $count$ fields in all ops are distinct, and as a set equal to $\{1,\ldots,n\}$.
- The $valc$ fields in all $del$ ops are distinct.
- If $(val,\read,valc,c)\in ops$, then $valc<c$ and $(val,\add,valc)\in ops$.
- If $(val,\del,valc,c)\in ops$ then $valc<c$ and $(val,\add,valc)\in ops$.

Let $V$ be set of records.
We say $O$ is \emph{consistent with $V$} if:
\begin{itemize}
 \item $O$ is consistent.
 \item $V=\set{(val,c) | (\add,val,c)\in O\;\&\; \forall c',(\del,val,c,c')\notin O }$. In words,
 $V$ is the set of notes that were added but not deleted.
\end{itemize}
