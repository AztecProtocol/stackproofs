...
\newcommand{\op}{\ensuremath{\mathscr{o}}\xspace}
...
\newcommand{\incsum}{\ensuremath{\mathrm{sum}}\xspace}
\newcommand{\inchash}{\ensuremath{\mathrm{h}}\xspace}
...
\subsection{Valid executions as RCG}\label{sec:exec->RFC}
...
Let \recset be a set of records and \ops be a set of record operations.
Then \ops has output \recset if and only if the following hold:
\begin{enumerate}
 \item \[\sumi{n}\frac{1}{X+vY+c}=\sum_{i:\op_i=\add}\frac{1}{X+v_iY+c}-\sum_{i:\op_i=\del}\frac{1}{X+v_iY+c}\]
 ...
 \end{enumerate}
 we have
 \[\sumi{n}\frac{\delta_i}{X+v_i Y +c_i}\]
\end{enumerate}
\end{claim}
...
\item $\incsum^*=\incsum+\sumi{M}\frac{\delta_i+\epsilon_i \eta_i}{\alpha +\beta v_i+\ c_i}$
\item $\beta^*=\beta,\gamma^*=\gamma,\epsilon^*=\epsilon$.
\item $\inchash^*=\hash(\inchash,\cm(S_F,M))$
\end{enumerate}

$f^*(Z,V)=\acc$ if and only if
\begin{enumerate}
 \item $\incsum=\sum_{(v,c)\in V} \frac{1}{\alpha + \beta v+ c}$
...
Then  $((\zfin,C,\recset),\phi(\tree))\in \rel_{F,\finpred}$  iff $((\zfin^*,C,\recset)$.
...
Then e.w.p. \negl, $((\zfin,C,\recset),\phi(\wit))\in \rel_{F,f}$.
