...
\begin{remark}\label{rem:agm->ext commitments}
 ...
 For more details see \cite{agm} or Section 2.2 of \cite{plonk}.
\end{remark}
...
Let $\inst_1=(z_1,1,\acchash_1)$, $\omega_1=(\acc_0,\acc_0, \inst_0, w_1, \prf_0)$ for arbitrary values $\acchash_1,\acc_0,\prf_0$. (We can choose some values arbitrarily as they aren't constrainted in $F'$  when $\cnt=1$.)

\item Let $\acc_1$ be a randomly chosen satisfiable accumulator. Namley, $\acc_1=(\cm(\witacc_1),\betaa,e)$ where $\witacc_1$ and $\betaa$ are chosen randomly\footnote{According to \cm one may  choose only a subset of $\witacc_1$'s indices randomly, and set the rest to zero such that $\cm(\witacc_1)$ is random.}, and $e$ is set to $e=\sum_{i\in [n]}\pow_i(\betaa)f_i(\witacc_1)$. 
