...
Checking the existance of an add operation with smaller timestamp than that of the read requires a constraint between the inputs of both circuits.
...
\adv outputs $(\inst,\tau),\inst',(\inststar,\tau^*),\wit,\wit',\witstar$
such that 
\begin{enumerate}
...
\item the witness 
$\witstar\defeq \gamma\cdot \wit +  \gamma\cdot \wit_1,$
\item and the proof $\prf\defeq (a,b)$.\\
\end{itemize}
\noindent
\underline{$\ver_{PG}(\insttbase,\inst_1, \instt,\pi=(a,b)):$}\noindent
\begin{enumerate}
 \item  Compute $\alpha,\beta,\delta,\gamma$ as in the prover algorithm given $\insttbase,\inst_1,a,b$.
...
For the knowledge soundness analysis we'll use a variant of the Zero-Testing assumption form \cite{novarecursive}.

\begin{dfn}\label{dfn:ZTA}
Fix $\cm:\F^M\to K$, hash function \hash, parameter $d$. The Zero-Testing Assumption (ZTA) for $(\hash,\cm,d)$ states the following. 

For any efficient \adv and efficiently computable function $D$ into \polysofdeg{d},
the probability of \adv outputting $x,\tau$ such that,
when setting
\begin{itemize}
 \item $f(X)=D(x,\tau)$
 \item $z=\hash(\cm(x),\tau)$
 ...
 we have that $f$ is not the zero polynomial but $f(z)=0$,
is \negl.
\end{dfn}



We extend $\cm$ to work 

\begin{thm}\label{thm:pgsoundness}
Set $d'\defeq \max \set{\log n,d}$. Denote by $\cmtwo$ the function $\cmtwo(\wit_1,\wit_2)\defeq (\cm(\wit_1),\cm(\wit_2))$.
Assume that \cm is collision resistant and the ZTA holds for $(\hash, \cmtwo,d')$
Then \protogal has knowledge soundness in the Algebraic Group Model.
\end{thm}
\begin{proof}
 Let $E$ be the event that \adv has output
...
\item $\ver(\inst,\inst_1,\inststar,\prf)=\acc$ 
...
probability \negl for any \adv.
We construct an efficient \advprime that when $E$ occurs outputs either a collision of 
\cm or an exception to ZTA. Hence, by the theorem assumption $E$ is contained in two events having
...
\[\cm( \gamma\wit+(1-\gamma)\wit')=\gamma \inst +(1-\gamma)\inst_1=\inststar=\cm(\witstar).\]
Thus, if $\witstar \neq \gamma \wit + (1-\gamma)\wit'$, \advprime can output $(\witstar,\gamma\wit+(1-\gamma)\wit')$
as a collision of \cm.
Now assume that $\witstar = \gamma \wit + (1-\gamma)\wit'$.
...
\[F_{ZTA}(X)\defeq F'(X)-\sum_{i\in [n]} \pow_i(\betaa+X\deltaa) f_i(\wit)\]
\[G'(X)\defeq F'(\alpha)X+ Z(X)K'(X)-\sum_{i\in [n]}\pow_i( \betaa^*) f_i(X\wit +(1-X)\wit_1)\]

Since $((\inststar,\betaa^*,e^*),\witstar)\in \relrand$ and $\ver_{PG}(\insttbase,\inst',\instt,\prf)=\acc$,
\[F'(\alpha)\gamma + Z(\gamma)K'(\gamma)=e^*=\sum_{i\in [n]} \pow_i(\betaa^*) f(\witstar).\]

This precisely means that $G'(\gamma)=0$.

If $(\inst_1,\wit_1)\notin \rel$
then (the honest) $G$ will have a $(1-X)\beta^* f(w_1)$ factor that can't be canceled out by $Z(X)K(X)$ so $G'$ can't be the zero polynomial (see PG paper analysis if not clear).

And hence taking $d=(w,w_1,\beta,\alpha,\delta,K), D(d)=G'$ 
we get a contradiction to ZTA. (A little care needs to be taken to define $h$ and $C$ so that we really get $h(C(w,w_1,\beta,\alpha,\delta,K))=\gamma$).

Now assume $( (\phi,\beta,e),w)\notin R_{rand}$.
And that $G'\equiv 0$ - otherwise we can contradict ZTA via $G'$ as above.

Thus, if we write $G'(X)=c_1 X + c_2(1-X)+Z(X)K'(X)$ for some constants $c_1,c_2$ and polynomial $K'$ we must 
have $c_1,c_2,K'=0$.
Looking at the equation of $G'$
we have $c_1=F(\alpha)-\beta^*f(w)$.

Note that $c_1=F'(\alpha)$, so $F'(\alpha)=0$.

We also have $F'(0)=e-\beta f(w)\neq 0$, and thus $F'$ is not the zero polynomial.

Hence, in this case  $d=(w,\beta,e,w_1,\delta)$
$D(d)=F'$ is a counterexample to ZTA.
