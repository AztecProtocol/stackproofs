...
This section introduces a model of the operations on the global state that result from executing a contract function.
For simplicity, we don't discuss various 
	The global state consists of notes  that can be added, read or deleted by  the contract functions; and that a 
	...
	first added).
	To this end, Section \ref{sec:exec_model:record_ops} defines \emph{record operations} which keep track of the computation steps at which
	notes were added, read and deleted. Then, it defines conditions on record operations that guarantee the consistency of the operations 
...
The executed function is represented in the instance by a single group element \f. In the terminology of \cite{plonk}, \f is a commitment to the permutation and selector values.
...
We stress, however, that the \emph{interpretation} of these values in the instance as record operation and inner calls, only happens in the next section when we discuss valid executions,
and doesn't manifest in the defintion of \relapp below.
