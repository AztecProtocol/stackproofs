...
But the call to ``authorize'' need not be tied to a specific implementation and consequently,  to averification key -- as different token holders are 
...
We can thus assign a \emph{counter} equal to one to the first operation \Patrick{equal to one for the first operation}, and increment the counter with
each operation.
We then need to check, for example, that if a note was read \Patrick{or deleted} with a certain counter, it was indeed added
...
part of the witness to generate a random challenge. As eluded \Patrick{alluded?} to earlier,
...
\Patrick{Suggestion:This section introduces a structure on the operations on the global state that result from executing a contract function.
	Recall that the global state consists of notes (which are simply field elements) that can be added, read or deleted by functions, and that a 
	function can call other functions.
	As the called functions modify themselves the global state (and for instance delete a note that was added during the execution), an ordering on 
	...
	first added).
	To this end, Section \ref{sec:exec_model:record_ops} defines record operations which essentially keep track of the computation steps at which
	notes were added, created and deleted, and then defines conditions on record operations that guarantee the consistency of the operations 
	performed by the functions throughout the execution.
}

\Patrick{
	Suggestion-second paragraph:
	The goal is to eventually prove that a set of operations results from the execution of a function (that may call other functions).  
	The proof should not reveal any information about which precise function, as long as it belongs to a pre-defined set of contract functions,
	or about its arguments.
	To do so, it is sufficient to prove knowledge of an execution tree with the initial function and its arguments (which include the new declared
	global state as parts of its inputs) at the root, the functions it calls and their arguments at its children nodes and so forth. 
	The execution of the initial function is valid if the function at each node accepts on its arguments.
	Section \ref{sec:exec_model:relation} defines an NP relation which formally captures what it means for a function to accept on its arguments,
...
\Patrick{Suggestion: In particular, functions are represented by commitments which consist of a single group element.}
...
\Patrick{Suggestion:
	To enable efficient proofs that a new global state stems from the execution of a contract function, we introduce Repeated Computations with 
	Global state (RCG).
	They split an overall function execution into stepwise atomic executions of each of the functions called throughout the execution. 
	A transition predicate attests to the validity of a global-state transition arising from an atomic execution.
	A final predicate checks the consistency between parts of the witnesses used throughout the entire computation.
	These parts are later interpreted as record operations performed by the functions called. 
	An RCG scheme is then defined as a SNARK in which the prover need only keep in memory enough information to check each stepwise transition and
	to check the final predicate.
}
\Patrick{Formally,} An RCG relation is defined by a pair of functions $(F,\finpred)$.\\
\noindent
...
\begin{itemize}
 ...
 \item $\finpred(z_n,s_1,\ldots,s_n,\recset)=\acc$.
	 \Patrick{the input space of $\finpred$ in the def above does not include values $z.$}
\end{itemize}
...
\Patrick{We now define specific RCG transition and final relations, and then reduce proving knowledge of an execution tree with the new global state as
	arguments at the root to proving the validity of an RCG computation which results in a pre-determined final state.
	This final state that reveals no information beyond the fact that the new global state arising from the execution of a single contract
	function.
}
...
\item If $\init=\true$, $g$ contains exactly one element.
 \item Denoting $g[0]=(\f,\args)$, we have $\f=\instnoops.\f$ and $\args=\instnoops.\args$.
	 \Patrick{Does $\instnoops$ only have two components or more? To which set does it belong? Update: This is implicitly answered in the next 
	 item. Swap the two}
...
the stack obtained by popping $g[0]$ \Patrick{$g_{i-1}[0]$?} and adding $(\instapp_i.\f_j,\instapp_i.\args_j)$ for $j\in [\instapp_i.\callnum]$.
...
\Patrick{
	We give rational identities which are equivalent to the consistency of record operations during a function execution, as formally defined 
	...
	We then define new RCG transition and final relations which allow to efficiently prove that the record operations are consistent. 
	Proving the validity of a computation with the RCG relations defined in Section \ref{sec:exec->RFC} is then reduced to proving the validity of 
	a computation with these new RCG relations.
}
...
\Patrick{
	We now define new RCG relations $\rel_{F,\finpred}$ which permit efficient proofs that record operations are consistent. 
	To that end, the previous rational identities are evaluated at random points and the transition predicate checks that each identity summand 
	was added one term at a time.
	For the random evaluation points to be generated in manner that is sound but also allow for an incremental computation of the rational identity 
	summand by summand, the idea is to compute them via a hash chain.
	...
	operations at each step, is used to compute the evaluation points. 
	This idea also appears in CITE MANGROVE, ongoing work by Jens Groth \cite{jenstalk} and CITE THE BLOG POST JENS MENTIONS IN HIS TALK.
}
...
Suppose an efficient adversary \adv outputs \wit such that $(\inst,\wit)\in \rel_{F^*,\finpred^*}$.
