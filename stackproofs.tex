...
Then $((\root,C,\recset),(n,\tree))\in \relexec$ if and only if $((\zfin,C,\recset),\varphi(\tree))\in \relrcg$.
\end{lemma}
\begin{proof}
...
It is left to show that e.w.p. \negl $\finpred(z_n,s_1,\ldots,s_n,\recset)=\accept$.

From $((\zfin^*,C,\recset),\wit)\in \relrcgstar$ we know the equations from Claim \ref{clm:reducetologder} hold
...
\paragraph{The relation $\relrand$.}
 ...
 Let $t\defeq \log n$. For $i\in [n]$, let $S_i\subset \set{0,\ldots,t-1}$ be the set such that $i-1=\sumj{S_i}2^j$. We define the $t$-variate polynomial $\pow_i$ as
...
\item Check that $\instt$ is computed as in the prover algorithm. Output \accept iff this is the case.
\end{enumerate}
...
% Since $\ver_{PG}(\insttbase,\inst_1,\instt,\prf)=\accept$, we have $\inststar=\gamma \inst +(1-\gamma)\inst_1$.
...
Since $((\inststar,\betaa^*,e^*),\witstar)\in \relrand$ and $\ver_{PG}(\insttbase,\inst',\instt,\prf)=\accept$,
\begin{align*}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $\verpg(\insttbase,\instFprime_0,\instt,\prf)=\accept$.
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
 \item Let
$\inpF =(\zfin,C,\recset),\witF=(n,(z_0,\ldots,z_n),(w_1,\ldots,w_n))$. Recall that $(\inpF,\witF)\in \relrcg$ implies $F(z_{i-1},w_i,z_i)=\accept$ for each $i\in [n]$, $z_0.\init =\true, z_n=\zfin$ and $\finpred(\zfin,\recset) = \accept$.
% \item For $i\in [n]$, let
...
\item Output $\prf=( (h_n,\alpha,\beta,\eps),\prf^*)$.
\end{enumerate}

\noindent
\underline{$\ver(\instexec,\prf)$:}
\begin{enumerate}
\item Parse $\prf$ as $(\inchash,\alpha,\beta,\eps,\prf^*)$. Parse $\instexec$ as $(\zfin,C,\recset)$. Define
$\instrcg=((\zfin,\inchash,\alpha,\beta,\eps),C,\recset)$.
 \item Let $\ver'$ be \ver from Section \ref{subsec:main}. Return $\ver'(\instrcg,\prf^*)$.
\end{enumerate}
