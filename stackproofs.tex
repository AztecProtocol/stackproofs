...
We define $Z(X)\defeq X\cdot(1-X)$.
...
Fix $\cm:\F^M\to K$, hash function \hash, and integer $d$.
We say the tuple $(D,x,\tau)$ is a \emph{degree $d$-relation for $(\hash,\cm)$} if
\begin{enumerate}
 \item $D$ is a \poly-size circuit outputting elements of \polysofdegeq{d}.
 \item $f(X)\defeq D(x,\tau)$ is a non-zero polynomial. 
 \item Setting $z\defeq \hash(\cm(x),\tau)$, we have $f(z)=0$.
\end{enumerate}


The Zero-Testing Assumption (ZTA) for $(\hash,\cm,d)$ states that for any efficient \adv, the probability  that 
\adv outputs a degree $d$-relation for $(\hash,\cm)$ is \negl.
\end{dfn}
...
\[G'(\gamma) = F_0(\alpha)\gamma + Z(\gamma)K'(\gamma)- \sum_{i\in [n]} \pow_i(\betaa^*) f(\gamma\wit +(1-\gamma)\wit_1)=e^*-\sum_{i\in [n]} \pow_i(\betaa^*) f(\witstar)=e^*-e^*=0.\]


Set $x\defeq (\wit,\wit_1)$
Set $\tau\defeq (\insttbase,\inst_1,\prf)$
If $G\not\equiv 0$ \advprime outputs the degree $d'$-relation $(D,x,\tau)$,
where $D$ is the function that computes $G'$ according to \perpg given $\insttbase,\inst_1,\prf$

Assume now that $G\equiv 0$.

If $(\inst_1,\wit_1)\notin \rel$, using $Z(0)=0$ we have 
\[G'(0)= \sum_{i\in [n]}\pow_i( \betaa^*) f_i(\wit_1)=0\]
Define the polynomial $A(X,Y)\defeq \sumi{n}f_i(\wit_1)\pow_i(\betaa_1+X,\betaa_2 +XY,\ldots,\betaa_t+XY^{2^{t-1}})$.
Thus we have $A(\alpha,\delta)=0$. But since $(\inst_1,\wit_1)\notin 0$ $A$ is a combination of linearly independent monomials $\pow_i(\betaa_1+X,\betaa_2 +XY,\ldots,\betaa_t+XY^{2^{t-1}})$ with at least one non-zero coefficient and so $A(X,Y)\not\equiv 0$. Hence setting $D$ to be the function that computes $A$ according to \perpg given $\insttbase,\inst_1$, 
\advprime can output the degree $n$ relation $(D,x,(\insttbase,\inst_1))$.


Now, assume that $(\insttbase,\wit)\notin \relrand$.
As we're still assuming $G'\equiv 0$, we have 
\[G'(1)=F_0(\alpha)-\sumi{n}\pow_i(\betaa^*) f_i(\witstar)=0,
\]
But we also have $F'(\alpha)=G(1)$ and so $F'(\alpha)=0$.
On the other hand
\[F'(0)=e-\sumi{n}\pow_i(\betaa^*) f_i(\witstar)\neq 0\]
Setting $\tau=(\insttbase,\inst_1)$ and $D$ to be the function that computes $F'$ given $x,\tau$ we
