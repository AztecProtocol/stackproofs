...
This however, doesn't deal with the notes. During execution, note operations happened in a certain order.
We can think of these operations as having counters incremented by one with each operation.
We need to check, for example, that if a note was read with a certain counter, it was indeed added
with a smaller counter.
We can include a description of the note operations peformed by a function  in the public inputs of its circuit.
...
\begin{enumerate}
 ...
 \item Let $n=\inst.\cnt$. Define $\acc_n\defeq \acc, \inst_n \defeq \inst.$
 \item Now for $i=n,\ldots ,1$:
 \begin{enumerate}
  \item If $\acc_{i},\inst_{i}\in K$, let $\witacc_{i},\witinst_{i}$ be the elements output by \adv such that $\acc_i= \cm(\witacc_{i}),\inst_i=\cm(\witinst_i)$. Let $\inst_i=(z_i,\cnt_i,\hashacc_i)$,$\witinst_i=(\acc'_i,\acc_{i-1},\inst_{i-1},w_i,\prf_i)$
 \end{enumerate}
\item Output $\witrcg\defeq ( n,(z_1,\ldots,z_n),(w_1,\ldots,w_n))$.
\end{enumerate}
Look at following bad events:
\begin{enumerate}
 \item For some $i\in [n]$, $\acc'_i\neq \acc_i$.
 \item For some $i\in [n]$, $(\inst_i,\witinst_i)\notin \rel$.
 
\end{enumerate}
We argue that if these events didn't happen $(\instrcg,\witrcg)\in \relrcg$:
