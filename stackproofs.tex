...
at $\alpha,\beta,\eps$, i.e, defining the rational function
\begin{align*}
...
Let $f\in\F_{\leq d}[X,Y,Z]$ denote the resulting non-zero polynomial when multiplying $r(X,Y,Z)$ with all of its denominators. Note that $f(\alpha,\beta,\eps)=0$. Define $D$ as the function that computes $f(X,Y,Z)$ given $x\defeq\wit$, $\tau\defeq\recset$, and set $\ztafuncs\defeq\{D\}$. Then \advprime outputs the degree-$d$ relation $(D,x,\tau)$, which has probability \negl.
\end{proof}
...
\end{enumerate}
 ...
 \newcommand{\witacc}{\ensuremath{\mathsf{w}}\xspace}
...
\begin{enumerate}
 \item Let
$\inpF =(\zfin,C,\recset),\witF=(n,(z_0,\ldots,z_n),(w_1,\ldots,w_n))$. Recall that $F(z_{i-1},w_i,z_i)=\accept$ for each $i\in [n]$. 
% \item For $i\in [n]$, let 

\item Let $\acc_1$ be a randomly chosen satisfiable accumulator. Namley, $\acc_1=(\cm(\wit),\betaa,e)$ where $\wit$ and $\betaa$ are chosen randomly\footnote{According to \cm one may  choose only a subset of \wit's indices randomly, and set the rest to zero such that $\cm(\wit)$ is random.}, and $e$ is set to $e=\sum_{i\in [n]}\pow_i(\betaa)f_i(\wit)$. 
