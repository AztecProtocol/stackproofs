...
If $D$ has probability \negl we are done. We assume from now this isn't the case, and look at the probability space conditioned on $D$.



Let $E_0$ be the event  $(\instfin,\witfin)\notin \relfin$.
For $i\in [n]$, let $E_i$ be the event $(\inst_i,\witinst_i)\notin \rel$ or $(\acc_i,\witacc_i) \notin \rel$ or $\hash(\acc_i)= \inst_i.\hash$.
%  \item  $D_i:\acc'_i\neq \acc_i$.
...
the probability that $(\inst_i,\witinst_i)\notin \rel$ \emph{or} $(\acc_i,\witacc_i)\notin \relrand$ is \negl.
...
From $(\inst_{i},\witinst_{i})\in \rel$ we have that $\inst_{i}.\hash = \hash(\acc'_{i})=\hash(\acc_i)$.
If the probability that $\acc_i\neq \acc'_i$ is not \negl, we can construct an adversary $\adv'_i$ outputting $\acc_i,\acc'_i$ for the collision resistance of \hash.
...
From the knowledge soundness of \protogal applied to $\adv_i$ we have that the probability that $(\inst_{i-1},\witinst_{i-1})\in \rel$ or $(\acc_{i-1},\witacc_{i-1})\notin \relrand$ given $D_i$ is \negl.
In other words, the probability of $E_{i-1}$ given $D_i$ is \negl, which is the inductive statement.
