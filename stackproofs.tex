...
\begin{itemize}
 \item $Z=(Z_F,\inchash,\incsum,\beta,\gamma,\epsilon)$.
 \item $Z^*=(Z_F^*,\inchash^*,\incsum^*,\beta^*\gamma^*,\epsilon^*)$.
 \item $W=(W_F,S_F,M)$
\end{itemize}
...
\item $\incsum^*=\incsum+\sumi{M}\frac{\delta_i+\epsilon_i \eta_i}{v_i+\beta c_i + \gamma}$
...
\begin{enumerate}
 \item $\incsum=\sum_{(v,c)\in V} \frac{1}{v+\beta c +\gamma}$
 \item $\hash(\inchash,V)=(\beta,\gamma,\epsilon)$
\end{enumerate}
...
Define the polynomial $A(X)\defeq \sumi{n}f_i(\wit_1)\pow_i(\betaa_1+\alpha,\betaa_2 +\alpha X,\ldots,\betaa_t+\alpha X^{2^{t-1}})$.
We have $A(\delta)=0$. But since $(\inst_1,\wit_1)\notin \rel$, $A$ is a combination of the $n$ linearly independent polynomials $\sett{\pow_i(\betaa_1+\alpha,\betaa_2 +\alpha X,\ldots,\betaa_t+\alpha X^{2^{t-1}})}{i\in [n]}$ with at least one non-zero coefficient, and so $A(X)\not\equiv 0$. Hence, setting $D_2$ to be the function that computes $A$ given $(x,\insttbase,\inst_1,\alpha)$, 
