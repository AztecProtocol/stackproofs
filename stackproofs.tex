...
\begin{thm}
$(\prvexec,\verexec)$ is knowledge-sound for \relexec.
\end{thm}
\begin{proof}
Let \adv be a recursive algebraic adversary that outputs $\instexec=(\root,C,\recset)$ and $\prf=(\inchash,\incsum,\alpha,\beta,\eps,\prf^*)$ such that $\verexec(\instexec,\prf)=\accept$. Let $\zfin=(\empty,\root,\false)$, $\zfin^*=(\zfin,\inchash,\incsum,\alpha,\beta,\eps)$ and $\instrcg^*=(\zfin^*,C,\recset)$.

From the knowledge soundness of $(\prv,\ver)$ for \relrcgstar from Section \ref{subsec:main}, we can use the respective extractor \ext to obtain the witness $\witrcg^*$ such that $(\instrcg^*,\witrcg^*)\in\relrcgstar$ e.w.p. \negl.

Let $(\instrcg,\witrcg)=\varphi^*(\instrcg^*,\witrcg^*)$. From Lemma \ref{lem:FtoFstar}, we get $(\instrcg,\witrcg)\in\relrcg$ e.w.p. \negl.

Let $\witexec=\varphi^{-1}(\witrcg)$. From Lemma \ref{lem:execasRCG}, we get $(\instexec,\witexec)\in\relexec$ e.w.p. \negl, finishing the proof.
\end{proof}
