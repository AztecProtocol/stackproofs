...
\begin{remark}
The validity of ZTA is very sensitive to the choice of $D$. At first thought, one would want to prohibit $D$ from \emph{any} invocation of \hash or \cm. However, as will be seen later, this will be over-restrictive for capturing what is needed in multi-round protocols. We make sure not to use $D$ where \hash is invoked on $(\cm(x),\tau)$, and conjecture this suffices for avoiding couter-examples. In any case, analyzing a hash-based protocol under ZTA with a specific choice of $D$  allows for falsifiability - as opposed to an analysis in the ROM followed by applying Fiat-Shamir .
\end{remark}
...
% 	 value.}
 ...
 \item $\finpred(\zfin,s_1,\ldots,s_n,\recset)=\accept$.
\end{enumerate}
...
\item $\ver((\inst,\tau),\inst',(\inststar,\tau^*),\prf)=\accept$,
...
if there are efficiently computable and invertible maps $\varphi_0, \varphi_1$ such that
\begin{enumerate}
 \item $\varphi_0(\inst)$ is always of the form $(\inst,X)$ for some $X$.
 \item Any efficient \adv has a \negl probability of outputting a pair $(\inst,\wit)$ such that either:
\begin{enumerate}
...
We fix $\predinst,\cm,\varphi_0,\varphi_1$ to be the values appearing in the statement of Claim \ref{clm:repplonk}.
...
\begin{enumerate}
 \item $(\acc_i,\witacc_i,\prf_i) = \prvpg(\acc_{i-1},\witacc_{i-1},\varphi_0(\inst_{i-1}),\varphi_1(\witinst_{i-1}))$
\item $\inst_i = (z_i,i,\hash(\acc_i))$,
\item $\witinst_i=(\acc_i,\acc_{i-1},\inst_{i-1},w_{i},\prf_i)$
\end{enumerate}
\item
$(\acc,\witacc,\prf^*) = \prvpg(\acc_{n},\witacc_{n},\varphi_0(\inst_{n}),\varphi_1(\witinst_{n}))$.
\item Let $\prffin = \prvsnark(\instfin,\witfin)$ where
\begin{itemize}
 \item $\instfin = (\acc,\zfin,C,\recset)$
 \item $\witfin = (\witacc,\acc_n,\varphi_0(\inst_n),\prf^*)$
\end{itemize}
...
\begin{enumerate}
 \item Given $(\instrcg,\prffin)$, if $\prffin$ is not of the form $(\prf_1,\prf_2)$ with $\prf_1\in K$ output \empt and abort. Otherwise let $\witfin$ be the element output by \adv such that $\cm(\witfin)=\prf_1$.
 ...
 \item If $(\instfin,\witfin)\notin \relfin$, output \empt and abort. Otherwise, $\verpg(\acc_n,\inst_n,\acc,\prf^*)=\accept$, and thus $\acc_n,\inst_n\in K$. Let $\witacc_n,\witinst_n$ be the elements output by \adv such that $\cm(\witacc_n)=\acc_n$ and $\cm(\witinst_n)=\inst_n$.
 ...
 \begin{enumerate}
  ...
  \item Parse $\varphi_0^{-1}(\inst_i)$ as $(z_i,\cnt_i,\acchash_i)$. Parse $\varphi_1^{-1}(\witinst_i)$ as $(\acc'_i,\acc_{i-1},\inst_{i-1},w_i,\prf_i)$. Note that through this parsing we have in particular defined $z_i,w_i$ and $\inst_{i-1}$.
 \end{enumerate}
...
In this case $(\acc,\witacc)\in \relrand$, $\hash(\acc_n)=\inst_n.\acchash$ and $\verpg(\acc_n,\inst_n,\acc,\prf^*)=\accept$.
...
Now assume the induction hypothesis for some $i>1$. We have by the hypothesis that the event $D_i\defeq \neg(E_0\cup E_i\cup\ldots\cup E_n)$ has probability $1-\negl$. Assume that we are in $D_i$.
 From $\neg E_i$ we have $\inst_{i}.\acchash=\hash(\acc_i)$. From $(\inst_{i},\witinst_{i})\in \rel$ we have that $\inst_{i}.\acchash = \hash(\acc'_{i})$.
Thus $\hash(\acc_i)=\hash(\acc'_i)$.
From the collision resistance of \hash, we thus have that the probability that $\acc_i\neq \acc'_i$ is \negl.
If $\acc_i=\acc'_i$ we have from $(\inst_i,\witinst_i)\in \rel$ that $\ver(\acc_{i-1},\inst_{i-1},\acc_i,\prf_i) =\accept$.
Define $\adv_i$ to be the algorithm that executes \ext and outputs $\acc_{i-1},\inst_{i-1},\acc_{i},\witacc_{i-1},$ $\witinst_{i-1},\witacc_i,\prf_i$.
