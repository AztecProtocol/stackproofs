...
\newcommand{\accprev}{\ensuremath{}\xspace}
\begin{document}
    \maketitle
\begin{abstract}
...
\begin{itemize}
 \item $Z=(Z_F,\inchash,\incsum,\alpha,\beta,\varepsilon)$.
 \item $Z^*=(Z_F^*,\inchash^*,\incsum^*,\alpha^*,\beta^*,\varepsilon^*)$.
 \item $W=(W_F,S_F,M)$
\end{itemize}
\noindent
...
$\incsum^*=\incsum\;+$
\[\sum_{i\in [m];\op_i = \add}\frac{1+ \eps m_i}{\alpha +\beta v_i+\ c_i}-\sum_{i\in [m];\op_i = \read}\frac{ \eps }{\alpha +\beta v_i+vc_i}-\sum_{i\in [m];\op_i = \del}\frac{ 1 }{\alpha +\beta v_i+ vc_i}+ \sumi{m}\frac{\varepsilon^2}{\alpha+c_i}.\]
\item $\alpha^*=\alpha, \beta^*=\beta,\varepsilon^*=\varepsilon$.
\item $\inchash^*=\hash(\inchash,\cm(S_F,M))$. \\
\end{enumerate}

\noindent $\finpred^*(Z,\recset)=\acc$ if and only if
\begin{enumerate}
 \item $\incsum=\sum_{(v,c)\in \recset} \frac{1}{\alpha + \beta v+ c}+\sumi{n}\frac{\varepsilon^2}{\alpha+i}$,
 \item $\hash(\inchash,\recset,1)=\alpha$, $\hash(\inchash,\recset,2)=\beta$, $\hash(\inchash,\recset,3)=\varepsilon$.
\end{enumerate}
...
There is an efficiently computable and efficiently invertible map $\varphi$ such that the following holds.
Let \funcs be a set of function commitments with Merkle root \root. Fix positive integers $n,C$ with $n\leq C$.
Fix some $\alpha,\beta,\varepsilon\in \F$ and set of records \recset.
Define $\zfin=(\empty,\root,\false), \zfin^*\defeq (\zfin,\inchash,\incsum,\alpha,\beta,\varepsilon)$ and set $\inst\defeq (\zfin^*,C,\recset)$. Assume the ZTA for the appropriate family of functions appearing in the proof.
...
$z_i'=(z_i,\inchash_i,\incsum_i,\alpha_i,\beta_i,\varepsilon_i), w_i'=(w_i,s_i,M_i)$.
...
at $\alpha,\beta,\varepsilon$, i.e, defining the rational function
\begin{align*}
...
\end{align*}
we have $r(\alpha,\beta,\varepsilon)=0$. If any of the three rational identities from Claim \ref{clm:reducetologder} does not hold, we have $r(X,Y,Z)\not\equiv 0$, which allows us to construct an adversary \advprime against the $3$-variate ZTA for $(\ztafuncs,\hash,\cm,d)$.

Let $f\in\F_{\leq d}[X,Y,Z]$ denote the resulting non-zero polynomial when multiplying $r(X,Y,Z)$ with all of its denominators. Note that $f(\alpha,\beta,\varepsilon)=0$. Define $D$ as the function that computes $f(X,Y,Z)$ given $x\defeq\wit$, $\tau\defeq\recset$, and set $\ztafuncs\defeq\{D\}$. Then \advprime outputs the degree-$d$ relation $(D,x,\tau)$, which has probability \negl.
\end{proof}
...
We say  a pair of functions $(F,\finpred)$ used to define an RCG relation $\rel_{F,\finpred}$ is \emph{trivial} if the $S$ variable in $F$ is not used, and accordingly \finpred depends only on $(\zfin,\recset)$.
...
As in \cite{nova,othernova}, we first describe an extended function $F'$ that includes the folding verifier. Loosely speaking, this is what enables bootstrapping a folding scheme into an IVC. We describe the function arguments first.\\
...
%  \newcommand{\verpg}{\ensuremath{\ver_{\cm,n}}\xspace}
 \newcommand{\prvpg}{\ensuremath{\prv_{\cm,n,f}}\xspace}
 \newcommand{\prvsnark}{\ensuremath{\prv_{\text{\calligra{fin}}}}\xspace}
 \newcommand{\versnark}{\ensuremath{\ver_{\text{\calligra{fin}}}}\xspace}

 Let $\rel,\relrand$ be relations for the \protogal version of the function $F'$ above.
...
\begin{itemize}
 ...
 \item $\witfin = (\witacc,\acc_0,\inst,\prf)$
\end{itemize}
such that
\begin{enumerate}
 \item $(\acc,\witacc)\in\relrand$.
\item $\verpg(\acc_0,\inst,\prf,\acc)=\accept$.
\item $\inst.z=\zfin$.
\end{enumerate}
...
We now describe the full prover for $\rel_{F,\finpred}$.\\

\underline{$\prv(\inpF,\witF)$:}
\begin{enumerate}
 \item Let
$\inpF =(\zfin,C,\recset),\witF=(n,z=(z_0,\ldots,z_n),w=(w_1,\ldots,w_n)$. Recall that $F(z_{i-1},w_i,z_i)=\accept$ for each $i\in [n]$.
% \item For $i\in [n]$, let
...
\begin{enumerate}
 \item $(\acc_i,\witacc_i,\prf_i) = \prvpg(\acc_{i-1},\witacc_{i-1},\inst_{i-1},\wit_{i-1})$
\item $\inst_i = (z_i,i-1,\hash(\acc_i))$,
\item $\wit_i=(\acc_i,\acc_{i-1},\inst_{i-1},w_{i},\prf_i)$
\end{enumerate}
\item
$(\acc,\witacc,\prf^*) = \prvpg(\acc_{n},\witacc_{n},\inst_{n},\wit_{n})$.
\item Let $\prf = \prvsnark(\instfin,\witfin)$ where
\begin{itemize}
 \item $\instfin = (\acc,\recset,C,\zfin)$
...
% ### The RC scheme (WIP)
% Let $P_{PG}(X,W,x,w)\rightarrow (X',W',\pi)$ be the protogalaxy prover for $F'$. We think of $P_{PG}$ as returning a tuple $(X',W',\pi)$ - respectively,
% the new accumulator instance, new accumulator witness, and a proof that the new accumulator is correct given the previous one and instance.
% $P$:
% Given $G, z_{final},F,n,z_0,\ldots,z_{n-1},\omega_0,\ldots,\omega_{n-1}$
%
% 1. Let $X_1$ be a randomly chosen satisfiable accumulator. Namley, $X_1=(\omega,\mathbf{\beta},e)$ where $\omega$ and $\beta$ are chosen randomly And $e$ is set to $e=\sum_{i\in [n]}pow_i(\beta)f_i(\omega)$.
% 2. Let $x_1=(z_1,G,0,hash(acc_{empty}))$, $w_1=(acc_{empty},acc_{empty}, inst_0, \omega_0, \pi)$. Here $inst_0$ satisfied $inst_0.z=z_0$, and doesn't need to be a satifiable instance.
%
% 2. For each $i=2..n$, compute
% $(X_i,W_i,\pi_i) = P_{PG}(X_{i-1},W_{i-1},x_{i-1},w_{i-1})$
% $x_i = (z_i,G,i-1,hash(X_i)), w_i=(X_i,X_{i-1},x_{i-1},\omega_{i-1},\pi_i)$
%
% 3. Output $(X_n,W_n,x_n,w_n)$.
%
% $V(G,z_{final}, (X,W,x,w))$:
% 1. Check that $(X,W)\in R_{rand}$, $(x,w)\in R$.
% 2. Check that $x.z=z_{final},x.acc =X,x.G=G,x.count\leq C$.
% ### Making the RC-scheme zero-knowledge
%
% We look at the relation $R_{final}$ (depending on $F$)
% defined as pairs $(x,w)$ where
%
% - $x=(z_{final},G,C,\phi^*)$
% - $w=(w^*,\phi,\phi_0,\pi)$
%
%
% such that
% - $\phi$ is an instance of $R_{acc}$, $\phi_0$ is an instance of $R$
% - $\phi_0.n\leq C$
% - $\pi$ is an accepting proof that $(\phi,\phi_0)$ fold into $\phi^*$
% - $(\phi^*, w^*)\in R_{acc}$.
% - $\phi_0.h=hash(\phi)$
% - $\phi.z=z_{final}$
%
...
% *The zk-prover:*
%
% 1. After running the [prover described above](#The-RC-scheme-WIP) for the RC-scheme, let $(X_n,W_n,x_n,w_n)$ be the final output of that prover.
% 2. Run the PG folding prover one more time to obtain $(\phi^*,w^*,\pi) = P_{PG}(X_{n},W_{n},x_{n},w_{n})$
% 3. Set $\phi=X_n, \phi_0=x_n$.
% 4. Create a zk-SNARK proof $\pi_{final}$ for $R_{final}$ with $x=(z_{final},G,C,\phi^*),w=(w^*,\phi,\phi_0,\pi)$.
% 5. Output $(x,\pi_{final})$.
%
%
% *It might be convenient to have $\pi_{final}$ consist of two zk-snark proofs sharing $\phi^*$ as a public input where one proof shows the existence of $w^*$ such that $(\phi^*,w^*)\in R_{acc}$ and the second proof the conditions in $R_{final}$ between $x$ and $w\setminus w^*$.*
%
%
% *Sketch of zk simulator:*
% $z_{final},G,C$ are part of the original input given to the simulator.
% Since $X_1$ was uniform, and $\phi^*=(cm,\beta,e)$ is the result of combining instances independent of $X_1$ into $X_1$,
% $cm$ is the commitment of a uniformly distributed $\omega$
% and $\beta\in F^t$ is uniformly distributed. Thus to simulate $\phi^*$ we can choose $\omega,\beta$ uniformly,
% and compute the resultant $e=\sum_{i\in [n]}pow_i(\beta)f_i(\omega)$.
% Then we can simulate $\pi_{final}$ using the zk-snark simulator.
%
% *Sketch of knowledge soundness:*
% Given $(x,\pi_{final})$ the snark extractor gives us
% the witness $w=(w^*,\phi,\phi_0,\pi)$
% so that $(x,w)\in R_{final}$
% Now, given  $w$ and $\phi^*$
% the folding extractor gives us the witnesses $w,w_0$ for $\phi,\phi_0$. $w_0$ will contain a previous accumulator $acc_{prev}$ and instance $inst$ such that $inst.count=\phi_0.count -1$, along with a proof $\pi$ that $acc_{prev}, inst$ fold into $\phi$. Using the folding extractor again we can obtain witnesses to $acc_{prev},inst$.
% $inst$'s witness will contain an instance with $count=\phi_0.count-2$ that folds into $acc_{prev}$. Continuing like this we can extract a full witness for the RC claim $(z_{final},G,C)$.
%
